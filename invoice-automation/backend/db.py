...
#     #     """Return all invoices, newest first with ObjectId converted to string."""
#     #     if self.db is None:
#     #        return []
#     #     try:
#     #        invoices = list(self.db[Config.INVOICES_COLLECTION].find().sort("_id", -1))
#     #        for inv in invoices:
#     #           inv["_id"] = str(inv["_id"])
...
#         return invoices  # ✅ correct indentation
...
class DatabaseManager:
    ...
    def get_dashboard_summary(self):
        ...
        try:
            ...
            categories = {
                item["_id"]: {
                    "total": float(f"{float(item['total']):.2f}"),
                    "count": item["count"]
                }
